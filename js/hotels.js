            ...
            <div class="relative bg-white rounded-2xl lg:rounded-[2rem] shadow-[0_8px_20px_rgba(0,0,0,0.04)] hover:shadow-[0_15px_40px_rgba(31,58,64,0.08)] border border-slate-100 transition-all duration-300 mb-6 lg:mb-8 group overflow-hidden animate-fade-in-up flex flex-col lg:flex-row mx-1 lg:mx-0" style="animation-delay: ${animationDelay}ms;">
              <div class="relative w-full lg:w-[320px] shrink-0 h-52 sm:h-64 lg:h-auto overflow-hidden p-2 lg:p-2.5 pb-0 lg:pb-2.5">
                <div class="w-full h-full rounded-xl lg:rounded-[1.2rem] overflow-hidden relative">
                    ...
                    <div class="absolute top-3 left-3 flex items-center bg-white/95 backdrop-blur-md rounded-xl shadow-lg overflow-hidden z-10 p-1 border border-white/50">
                        ...
                        <div class="px-2 flex items-center"><span class="text-[#1f3a40] font-extrabold text-[9px] md:text-[10px]">رائع</span></div>
                    </div>
              ...
              <div class="w-full lg:w-[270px] xl:w-[290px] p-4 lg:p-7 flex flex-col justify-center items-center bg-slate-50 shrink-0 z-10 relative overflow-hidden rounded-b-2xl lg:rounded-bl-[2rem] lg:rounded-br-none lg:rounded-tl-[2rem]">
                ...
                <div class="text-center w-full relative z-10 flex flex-col justify-center h-full">
                    <div class="hidden lg:block"><span class="inline-block bg-[#00b4d8]/10 text-[#00b4d8] text-[10px] font-black px-3 py-1 rounded-full mb-3 border border-[#00b4d8]/20 tracking-wide uppercase mx-auto w-fit">أفضل سعر متاح</span></div>
                    <div class="flex flex-row lg:flex-col items-center lg:justify-center justify-between w-full mb-3 lg:mb-0">
                        ...
                        <div class="w-[130px] lg:hidden">
                            <button class="w-full bg-gradient-to-l from-[#800000] to-[#a30000] active:scale-[0.98] transition-all text-white font-black py-3 rounded-xl shadow-[0_6px_15px_rgba(128,0,0,0.2)] border-none cursor-pointer text-xs flex items-center justify-center gap-1.5" onclick="${onClickHandler}">
                    ...
                    <div class="hidden lg:block w-full">
                        ${cashbackHTML}
                        <button class="w-full bg-gradient-to-l from-[#800000] to-[#a30000] hover:from-[#990000] hover:to-[#cc0000] active:scale-[0.98] transition-all duration-300 text-white font-black py-3.5 rounded-xl shadow-[0_6px_15px_rgba(128,0,0,0.2)] hover:shadow-[0_10px_20px_rgba(128,0,0,0.3)] border-none cursor-pointer text-sm flex items-center justify-center gap-2" onclick="${onClickHandler}">
    ...
    viewHotelDetails: async function(hotelName, basePrice, apiRooms, provider, hotelId) {
        ...
        let stickyHeaderHTML = `
            <div id="smartStickyBar" class="sticky top-[60px] lg:top-[70px] z-[50] bg-white/95 backdrop-blur-xl border border-slate-200 shadow-[0_15px_40px_rgba(31,58,64,0.12)] p-2.5 md:p-3 mb-6 rounded-2xl flex justify-between items-center opacity-0 translate-y-[-20px] pointer-events-none transition-all duration-500 mx-1 lg:mx-0">
                <div class="flex items-center gap-2 md:gap-3">
                    <div class="w-8 h-8 md:w-10 md:h-10 rounded-full bg-cyan-50 flex items-center justify-center shrink-0 border border-cyan-100"><i class="fa-solid fa-hotel text-[#00b4d8] text-xs md:text-sm"></i></div>
                    <div>
                        <h3 class="text-xs md:text-sm font-black text-[#1f3a40] line-clamp-1 max-w-[140px] sm:max-w-[220px]"><bdi dir="auto">${hotelName}</bdi></h3>
                        <p class="text-[9px] md:text-[10px] text-rose-600 font-bold mt-0.5 flex items-center gap-1"><span class="w-1.5 h-1.5 rounded-full bg-rose-500 animate-ping shrink-0"></span> يحظى باهتمام عالي الآن</p>
                    </div>
                </div>
                <div class="text-left flex items-center gap-3">
                    <div class="hidden sm:block text-right">
                        <span class="text-[9px] text-slate-400 block font-bold">السعر المبدئي</span>
                        <div class="flex items-baseline gap-1" dir="ltr"><span class="text-[10px] font-bold text-slate-400">AED</span><span class="text-lg font-black text-[#1f3a40] tracking-tighter">${basePrice}</span></div>
                    </div>
            ...
            effectiveRooms.forEach((room, rIdx) => {
                ...
                ratesArray.forEach((rate, rateIdx) => {
                    ...
                    roomsList.push({
                        ...
                        points: Math.floor(parseFloat(currentPrice) * 10),
                        policyText: rate.formattedPolicy || room.formattedPolicy || "شروط الإلغاء مطبقة وفق سياسة الفندق", 
                        isFreeCancel: rate.freeCancellation || room.freeCancellation || false,
                        paymentType: rate.paymentType || room.paymentType || 'AT', 
                        refundType: rType,
        ...
        roomsList.forEach((room, index) => {
            ...
            const bookingDataObj = {
                ...
                roomId: room.roomId,
                processKey: room.processKey,
                hotelName: hotelName.replace(/'/g, "\\'"),
                price: room.price,
                roomName: room.name.replace(/'/g, "\\'"),
                board: room.board,
                    ...
                    <div class="flex-1 p-5 lg:p-7 flex flex-col justify-between relative z-10 bg-white text-right">
                        <div>
                            <div class="inline-flex items-center gap-1.5 bg-rose-50 border border-rose-100 text-rose-600 px-2.5 py-1 rounded-full text-[10px] font-black mb-3">
                                <span class="w-1.5 h-1.5 rounded-full bg-rose-500 animate-ping"></span> ${room.urgency}
                            </div>
                            <h3 class="text-lg sm:text-xl md:text-2xl font-black text-[#1f3a40] leading-tight mb-2 flex items-center gap-2 justify-start"><bdi dir="auto">${room.name}</bdi></h3>
                            <div class="flex overflow-x-auto lg:flex-wrap items-center gap-2 mt-3 justify-start hide-scrollbar snap-x pb-1 lg:pb-0" style="-webkit-overflow-scrolling: touch;">
                                <span class="inline-flex items-center gap-1.5 bg-slate-50 text-slate-700 px-3 py-1.5 rounded-lg text-[10px] md:text-[11px] font-bold border border-slate-100 shrink-0 snap-center lg:snap-align-none"><i class="fa-solid fa-bed text-[#00b4d8]"></i> ${room.bed}</span>
                                <span class="inline-flex items-center gap-1.5 bg-slate-50 text-slate-700 px-3 py-1.5 rounded-lg text-[10px] md:text-[11px] font-bold border border-slate-100 shrink-0 snap-center lg:snap-align-none"><i class="fa-solid fa-user-group text-[#00b4d8]"></i> يتسع لـ 2 بالغين</span>
                                ${mealBadge}
                            </div>
                        </div>
