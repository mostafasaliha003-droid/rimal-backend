...
import { ArrowRight, BedDouble, ImageOff, MapPin, ShieldCheck, Star } from 'lucide-react';
...
export default function HotelDetails({ hid, onBack, displayCurrency, displayRates }) {
    ...
    return <main className="min-h-screen bg-remal-bg px-5 py-8 lg:px-10">
        <div className="mx-auto max-w-7xl space-y-7">
            ...
            <div>
                <div className="flex flex-wrap items-center gap-3 text-sm font-bold text-amber-700">
                    {Number(hotel?.stars || hotel?.star_rating) > 0 && <span className="inline-flex items-center gap-1"><Star size={16} />{hotel.stars || hotel.star_rating}</span>}
                    {hotel?.city && <span className="inline-flex items-center gap-1 text-slate-600"><MapPin size={15} />{hotel.city}</span>}
                </div>
                <h1 className="mt-2 text-3xl font-black text-remal-dark sm:text-4xl">{hotel?.name || 'Hotel'}</h1>
                {(hotel?.address || hotel?.staticData?.address) && <p className="mt-2 flex items-center gap-2 text-sm font-semibold text-slate-600"><MapPin size={15} className="shrink-0" />{hotel?.address || hotel?.staticData?.address}</p>}
            </div>

            <section aria-label="صور الفندق" dir="rtl" className={`grid gap-2 overflow-hidden rounded-2xl bg-slate-100 shadow-sm lg:h-[420px] ${images.length > 1 ? 'lg:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)]' : ''}`}>
                <div className="min-h-0 min-w-0 overflow-hidden">
                    {images[0] ? <img src={images[0]} alt={hotel?.name || 'الفندق'} className="h-64 w-full object-cover sm:h-80 lg:h-full" /> : <div className="flex h-64 items-center justify-center px-6 text-center text-slate-600 sm:h-80 lg:h-full">لا توجد صورة موثقة للفندق</div>}
                </div>
                {images.length > 1 && <div className="grid min-h-0 min-w-0 grid-cols-2 grid-rows-2 gap-2">
                    {Array.from({ length: 4 }).map((_, index) => images[index + 1] ? (
                        <div key={images[index + 1]} className="min-h-0 overflow-hidden"><img src={images[index + 1]} alt={`${hotel?.name || 'الفندق'} — صورة ${index + 2}`} loading="lazy" className="h-28 w-full object-cover transition-transform duration-500 hover:scale-105 motion-reduce:transform-none sm:h-40 lg:h-full" /></div>
                    ) : (
                        <div key={index} role="img" aria-label="لا توجد صورة إضافية موثقة" className="flex h-28 items-center justify-center bg-slate-100 text-slate-500 sm:h-40 lg:h-full"><ImageOff size={20} aria-hidden="true" /></div>
                    ))}
                </div>}
            </section>
            ...
            <section className="grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_20rem]">
                ...
                <aside className="min-w-0 self-start rounded-2xl bg-remal-dark p-6 text-white shadow-sm lg:sticky lg:top-24">
                    <p className="text-sm text-white/80">ملخص الإقامة</p>
                    <p className="mt-3 text-2xl font-bold">{rooms.length ? `${rooms.length} عروض إقامة` : 'لا توجد عروض متاحة'}</p>
                    <p className="mt-2 text-sm text-white/80">{searchParams.checkin || 'تاريخ الوصول'} إلى {searchParams.checkout || 'تاريخ المغادرة'}</p>
                    <div className="mt-6 flex items-center gap-2 text-sm text-emerald-200"><ShieldCheck size={18} /> التحقق من العرض قبل الدفع</div>
                </aside>
            </section>
        </div>
    </main>;
