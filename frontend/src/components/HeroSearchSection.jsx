...
import { ArrowLeft, LoaderCircle, MapPin, CalendarDays, Users, Search } from 'lucide-react';
...
export default function HeroSearchSection({ onSearch, initialSearch }) {
    ...
    return (
        <section className="relative isolate overflow-visible bg-slate-900 py-16 lg:py-24">
            {/* Background Layers for Depth */}
            <div className="absolute inset-0 -z-10 bg-[url('https://images.unsplash.com/photo-1518684079-3c830dcef090?auto=format&fit=crop&w=2200&q=85')] bg-cover bg-center opacity-80" />
            ...
            <div className="mx-auto flex max-w-7xl flex-col px-5 lg:px-10">
                <div className="max-w-2xl text-right text-white drop-shadow-lg mb-8">
                    <h1 className="max-w-xl text-4xl font-black leading-tight sm:text-5xl md:text-6xl tracking-tight">
                        رمال <span className="text-blue-400">وفِلّها</span>
                    </h1>
                    <p className="mt-4 max-w-lg text-lg font-medium leading-relaxed text-slate-200 opacity-90">إقامتك الجاية تبدأ هنا. اختر وجهتك وتواريخك وابحث عن أفضل العروض.</p>
                </div>

                <form onSubmit={handleSearch} className="relative w-full z-20" dir="rtl">
                    {/* The Masterstroke Floating Card */}
                    <div className="flex flex-col divide-y divide-slate-200/60 rounded-3xl border border-white/40 bg-white/95 p-2 shadow-[0_8px_30px_rgb(0,0,0,0.12)] backdrop-blur-xl lg:flex-row lg:items-stretch lg:divide-y-0 lg:rounded-[2.5rem]">
                        
                        {/* Destination Field */}
                        <label className={`relative flex min-h-[85px] flex-1 items-center gap-4 rounded-2xl lg:rounded-l-none lg:rounded-r-[2rem] px-6 py-4 transition-all duration-300 ${activeField === 'destination' ? 'bg-white shadow-[0_4px_20px_rgb(0,0,0,0.08)] z-10 scale-[1.02]' : 'hover:bg-slate-50/80 z-0'}`} htmlFor="destination-search">
                            <MapPin className={`h-7 w-7 shrink-0 transition-colors ${activeField === 'destination' ? 'text-blue-600' : 'text-slate-400'}`} />
                            <span className="flex min-w-0 flex-1 flex-col text-right">
                                <span className="text-[12px] font-black uppercase tracking-wider text-slate-800 mb-0.5">الوجهة</span>
                            ...
                            {loading && <LoaderCircle size={20} className="animate-spin text-blue-500" />}
                            {showSuggestions && query.trim().length > 1 && suggestions.length > 0 && (
                                <div id="destination-suggestions" role="listbox" aria-label="اقتراحات الوجهات" className="custom-scrollbar absolute inset-x-0 top-[95px] z-50 max-h-80 overflow-y-auto rounded-2xl border border-slate-100 bg-white p-2 text-right shadow-2xl">
                        ...
                        {/* Check-in Field */}
                        <label className={`relative flex min-h-[85px] flex-1 items-center gap-4 lg:border-r border-slate-200/50 px-6 py-4 transition-all duration-300 rounded-2xl lg:rounded-none ${activeField === 'checkin' ? 'bg-white shadow-[0_4px_20px_rgb(0,0,0,0.08)] z-10 scale-[1.02]' : 'hover:bg-slate-50/80 z-0'}`}>
                            ...
                            <span className="flex min-w-0 flex-1 flex-col text-right">
                                <span className="text-[12px] font-black uppercase tracking-wider text-slate-800 mb-0.5">تسجيل الوصول</span>
                                <DatePicker
                                    ...
                                    className="w-full cursor-pointer border-none bg-transparent p-0 pt-1 text-right text-base font-bold leading-5 text-slate-900 outline-none placeholder:text-slate-400 focus:ring-0"
                        ...
                        {/* Check-out Field */}
                        <label className={`relative flex min-h-[85px] flex-1 items-center gap-4 lg:border-r border-slate-200/50 px-6 py-4 transition-all duration-300 rounded-2xl lg:rounded-none ${activeField === 'checkout' ? 'bg-white shadow-[0_4px_20px_rgb(0,0,0,0.08)] z-10 scale-[1.02]' : 'hover:bg-slate-50/80 z-0'}`}>
                            ...
                            <span className="flex min-w-0 flex-1 flex-col text-right">
                                <span className="text-[12px] font-black uppercase tracking-wider text-slate-800 mb-0.5">تسجيل المغادرة</span>
                                <DatePicker
                        ...
                        {/* Guests Field */}
                        <div className={`relative flex min-h-[85px] flex-1 items-center gap-4 lg:border-r border-slate-200/50 px-6 py-4 transition-all duration-300 rounded-2xl lg:rounded-none ${activeField === 'guests' ? 'bg-white shadow-[0_4px_20px_rgb(0,0,0,0.08)] z-10 scale-[1.02]' : 'hover:bg-slate-50/80 z-0'}`}>
                            ...
                            <details className="min-w-0 flex-1 text-slate-900 group" onToggle={(e) => setActiveField(e.target.open ? 'guests' : null)}>
                                <summary className="cursor-pointer outline-none list-none">
                                     <span className="text-[12px] font-black uppercase tracking-wider text-slate-800 block mb-0.5">الضيوف</span>
                                ...
                                <div className="absolute inset-x-0 top-[95px] z-50 max-h-96 min-w-[300px] space-y-4 overflow-auto rounded-2xl border border-slate-100 bg-white p-5 shadow-2xl lg:left-0 lg:right-auto">
                        ...
                        <div className="p-2 lg:flex lg:items-center">
                            <button type="submit" disabled={loading} className="group relative flex min-h-[70px] w-full items-center justify-center gap-3 overflow-hidden rounded-[1.8rem] bg-blue-600 px-8 text-base font-black text-white shadow-[0_4px_14px_0_rgb(37,99,235,0.39)] transition-all duration-300 hover:bg-blue-700 hover:shadow-[0_6px_20px_rgba(37,99,235,0.23)] hover:-translate-y-0.5 disabled:cursor-wait disabled:opacity-60 lg:w-auto z-20">
