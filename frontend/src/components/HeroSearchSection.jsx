                ...
                <div className={`transition-all duration-300 z-50 ${isSticky ? 'fixed top-0 left-0 right-0 bg-white/80 backdrop-blur-2xl shadow-sm border-b border-slate-200/50 py-3 px-4 sm:px-6 lg:px-10 animate-in slide-in-from-top-2' : 'relative w-full'}`}>
                    <form onSubmit={handleSearch} className="relative w-full mx-auto max-w-7xl z-20" dir={direction}>
                        ...
                        <div className={`search-panel flex flex-col divide-y divide-slate-200/60 border backdrop-blur-xl lg:flex-row lg:items-stretch lg:divide-y-0 transition-all duration-300 ${isSticky ? 'rounded-2xl border-slate-200 bg-white p-1 shadow-lg lg:rounded-full lg:p-1.5' : 'rounded-3xl border-white/70 bg-white/95 p-2 shadow-[0_22px_60px_rgba(7,31,51,0.22)] lg:rounded-[2rem] lg:p-2.5'}`}>
                            ...
                            <div className={`relative flex min-h-[75px] lg:min-h-[80px] flex-1 items-center gap-4 lg:border-r border-slate-200/50 px-6 py-4 transition-all duration-300 ${isSticky ? 'rounded-xl lg:rounded-none' : 'rounded-2xl lg:rounded-none'} ${activeField === 'guests' ? 'bg-white shadow-[0_4px_20px_rgb(0,0,0,0.08)] z-10 scale-[1.02] ring-1 ring-blue-100' : 'hover:bg-slate-50/80 z-0'}`}>
                                ...
                                <details className="min-w-0 flex-1 text-slate-900 group" onToggle={(e) => setActiveField(e.target.open ? 'guests' : null)}>
                                    ...
                                    <div className="absolute inset-x-0 top-[90px] z-50 max-h-96 w-full max-w-full space-y-4 overflow-auto rounded-2xl border border-slate-100 bg-white p-5 shadow-2xl lg:left-0 lg:right-auto lg:w-[min(100%,24rem)]">
                                        {guests.map((room, index) => <fieldset key={index} className="space-y-4 border-b border-slate-100 pb-5"><legend className="mb-3 text-base font-black text-slate-800">غرفة {index + 1}</legend>
                                            <label className="flex flex-wrap items-center justify-between gap-2 text-sm font-bold text-slate-600">البالغون<input aria-label={`البالغون في غرفة ${index + 1}`} type="number" min="1" max="6" value={room.adults} onChange={event => setGuests(guests.map((group, position) => position === index ? { ...group, adults: Math.max(1, Math.min(6, Number(event.target.value))) } : group))} className="w-1/3 max-w-[6rem] rounded-lg border-slate-200 p-2.5 text-center font-bold outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500" /></label>
                                            <label className="flex flex-wrap items-center justify-between gap-2 text-sm font-bold text-slate-600">الأطفال<select aria-label={`الأطفال في غرفة ${index + 1}`} value={room.children.length} onChange={event => setGuests(guests.map((group, position) => position === index ? { ...group, children: Array.from({ length: Number(event.target.value) }, (_, child) => group.children[child] ?? '') } : group))} className="w-1/3 max-w-[6rem] rounded-lg border-slate-200 p-2.5 font-bold outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500">{[0, 1, 2, 3, 4].map(count => <option key={count}>{count}</option>)}</select></label>
                                            {room.children.map((age, child) => <label key={child} className="flex flex-wrap items-center justify-between gap-2 text-sm font-bold text-slate-600">عمر الطفل {child + 1} (عند الوصول)<select aria-label={`عمر الطفل ${child + 1} في غرفة ${index + 1}`} value={age} onChange={event => setGuests(guests.map((group, position) => position === index ? { ...group, children: group.children.map((value, childIndex) => childIndex === child ? (event.target.value === '' ? '' : Number(event.target.value)) : value) } : group))} className="w-1/3 max-w-[6rem] rounded-lg border-slate-200 p-2.5 font-bold outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"><option value="">اختر</option>{Array.from({ length: 18 }, (_, value) => <option key={value}>{value}</option>)}</select></label>)}
                        ...
                        {(error || suggestionError) && (
                            <div id="search-error" role="alert" className="absolute -bottom-16 left-4 right-4 mx-auto flex max-w-full flex-wrap items-center justify-center gap-2 rounded-full border border-red-100 bg-red-50 px-6 py-3 text-center shadow-lg">
