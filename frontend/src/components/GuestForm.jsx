      ...
      <form onSubmit={onSubmit} className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <input id="guest-first-name" name="given-name" autoComplete="given-name" maxLength={80} required type="text" className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-lg text-sm" value={guestDetails.firstName} onChange={(event) => onGuestDetailsChange({ ...guestDetails, firstName: event.target.value })} />
          </div>
          <div>
            ...
            <input id="guest-last-name" name="family-name" autoComplete="family-name" maxLength={80} required type="text" className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-lg text-sm" value={guestDetails.lastName} onChange={(event) => onGuestDetailsChange({ ...guestDetails, lastName: event.target.value })} />
          </div>
          <div>
            <label htmlFor="guest-email" className="block text-sm font-semibold text-slate-700 mb-2">البريد الإلكتروني</label>
            <input id="guest-email" name="email" autoComplete="email" required type="email" maxLength={254} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-lg text-sm text-left" dir="ltr" value={guestDetails.email} onChange={(event) => onGuestDetailsChange({ ...guestDetails, email: event.target.value })} />
          </div>
          <div>
            <label htmlFor="guest-phone" className="block text-sm font-semibold text-slate-700 mb-2">رقم الهاتف مع رمز الدولة</label>
            <input id="guest-phone" name="tel" autoComplete="tel" minLength={7} maxLength={25} required type="tel" className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-lg text-sm text-left" dir="ltr" value={guestDetails.phone} onChange={(event) => onGuestDetailsChange({ ...guestDetails, phone: event.target.value })} />
          </div>
        </div>

        {(guestDetails.rooms || []).map((room, roomIndex) => <fieldset key={roomIndex} className="space-y-4 border-t border-slate-200 pt-4"><legend className="text-sm font-bold">مسافرو الغرفة {roomIndex + 1}</legend>{room.guests.map((traveler, travelerIndex) => roomIndex === 0 && travelerIndex === 0 ? null : <div key={travelerIndex} className="grid gap-3 sm:grid-cols-2">{['firstName', 'lastName'].map(field => <label key={field} className="text-sm text-slate-700">{field === 'firstName' ? 'الاسم الأول' : 'اسم العائلة'} للضيف {travelerIndex + 1}{traveler.is_child ? ` (طفل، ${traveler.age} سنوات)` : ''}<input required maxLength={80} autoComplete="off" value={traveler[field]} onChange={event => onGuestDetailsChange({ ...guestDetails, rooms: guestDetails.rooms.map((group, groupIndex) => groupIndex === roomIndex ? { ...group, guests: group.guests.map((person, personIndex) => personIndex === travelerIndex ? { ...person, [field]: event.target.value } : person) } : group) })} className="mt-2 w-full rounded-lg border border-slate-300 p-3" /></label>)}</div>)}</fieldset>)}
        <label className="block text-sm text-slate-700">طلبات خاصة (اختيارية وغير مضمونة)<textarea value={guestDetails.specialRequests} maxLength={1000} onChange={event => onGuestDetailsChange({ ...guestDetails, specialRequests: event.target.value })} className="mt-2 w-full rounded-lg border border-slate-300 p-3" rows={3} /></label>
