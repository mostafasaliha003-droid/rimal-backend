import { ChevronDown, CircleUserRound, Gift, Menu, Search } from 'lucide-react';

const links = [
    { label: 'استكشاف الفنادق', active: true, icon: Search },
    { label: 'تتبع حجزك', active: false },
    { label: 'حصالة النقاط', active: false, icon: Gift }
];
            ...
            <div className="mx-auto flex h-[76px] max-w-7xl items-center justify-between px-5 lg:px-10">
                ...
                <nav className="hidden items-center gap-8 lg:flex">
                    {links.map(({ label, active, icon: Icon }) => (
                        <a key={label} href="#" className={`group flex items-center gap-2 border-b-2 py-7 text-[13px] font-bold transition ${active ? 'border-remal-blue text-white' : 'border-transparent text-white/55 hover:border-white/30 hover:text-white'}`}>
                ...
                <div className="flex items-center gap-2.5 text-xs font-bold">
                    ...
                    <button className="flex items-center gap-2 rounded-full bg-remal-red px-3 py-2 text-white transition hover:bg-[#a10b0b]" type="button">
