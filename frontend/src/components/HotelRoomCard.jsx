...
export default function HotelRoomCard({ room = {}, onBook }) {
    const name = room.name || 'جونيور سويت';
    const price = room.price || '433.06';
    return (
            ...
            <div className="flex flex-col gap-6 p-5 sm:p-6 lg:flex-row lg:items-center lg:justify-between">
                <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-5 flex flex-wrap gap-x-5 gap-y-3 text-xs font-bold text-slate-500">
                        <span className="flex items-center gap-2"><BedIcon size={17} className="text-remal-blue" /> سرير كينج</span>
                        <span className="flex items-center gap-2"><UsersIcon size={17} className="text-remal-blue" /> حتى 2 بالغين</span>
                        <span className="flex items-center gap-2"><Wifi size={17} className="text-remal-blue" /> واي فاي مجاني</span>
                    </div>
