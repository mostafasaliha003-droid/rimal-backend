...
function BookingSummary({ booking, room, total, currency, displayCurrency, displayRates, canPay }) {
    ...
    return (
        <aside aria-label="ملخص الحجز والسعر" className="order-1 h-fit lg:order-2 lg:sticky lg:top-28">
            <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-[0_18px_42px_rgba(15,35,55,0.10)]">
                <div className="relative overflow-hidden bg-[var(--remal-navy)] p-6 text-white sm:p-7">
                    ...
                    <div className="relative">
                        <p className="eyebrow text-cyan-200">ملخص الحجز</p>
                ...
                <div className="space-y-6 p-6 sm:p-7">
                    <div className="grid grid-cols-2 gap-3">
                        <div className="rounded-2xl border border-slate-100 bg-slate-50 p-4">
                            <p className="text-[11px] font-black text-slate-500">تسجيل الوصول</p>
                        ...
                        <div className="rounded-2xl border border-slate-100 bg-slate-50 p-4">
                            <p className="text-[11px] font-black text-slate-500">تسجيل المغادرة</p>
                    ...
                    <div>
                        <div className="flex items-end justify-between gap-4 border-b border-slate-100 pb-5">
                            <div>
                                <p className="text-sm font-black text-slate-900">تفصيل السعر</p>
                                <p className="mt-1 text-xs font-medium text-slate-500">بالعملة الأصلية للعرض</p>
                            </div>
                        ...
                        {localTaxes.length > 0 && (
                            <div className="mt-4 space-y-2 rounded-2xl border border-orange-100 bg-orange-50 p-4">
                                <p className="border-b border-orange-200/70 pb-2 text-xs font-black text-orange-900">رسوم محلية تُدفع في الفندق</p>
                    ...
                    <div className="rounded-2xl border border-cyan-100 bg-cyan-50 p-4" aria-live="polite" aria-atomic="true">
                        <p className="text-xs font-bold text-cyan-900">الإجمالي النهائي</p>
                    ...
                    <div className="space-y-3">
                        <p className="flex items-start gap-3 rounded-2xl border border-emerald-100 bg-emerald-50 p-4 text-xs font-bold leading-6 text-emerald-900"><ShieldCheck size={18} className="mt-0.5 shrink-0 text-emerald-700" />سيتم تحويلك إلى بوابة Ziina الآمنة بعد التحقق من السعر والتوفر.</p>
                        <p className="flex items-start gap-3 rounded-2xl border border-slate-100 bg-slate-50 p-4 text-xs font-bold leading-6 text-slate-700"><CreditCard size={18} className="mt-0.5 shrink-0 text-[var(--remal-blue)]" />{canPay ? <>عملة العرض: <strong className="mx-1">{currency}</strong>. سيتم خصم هذا المبلغ عبر Ziina، وتظهر الرسوم المحلية غير المشمولة أعلاه.</> : <>عملة العرض: {currency || 'غير محددة'}. الدفع الإلكتروني لهذا العرض غير متاح حالياً.</>}</p>
                    </div>
...
export default function Checkout({ booking, onBack, displayCurrency, displayRates }) {
    ...
    if (paymentReturn) {
        ...
        return (
            <main className="mx-auto flex min-h-screen max-w-2xl items-center justify-center bg-[#F8FAFC] px-5 py-24">
                <div className="w-full rounded-3xl border border-slate-100 bg-white p-8 text-center shadow-xl sm:p-10">
                    ...
                    {paymentReference && <div className="mt-8 rounded-2xl border border-slate-100 bg-slate-50 p-4"><p className="text-xs font-bold text-slate-400">مرجع المتابعة</p><p className="mt-1 font-mono text-sm font-bold text-slate-700">{paymentReference}</p></div>}
                    <div className="mt-10 flex flex-col items-center justify-center gap-4 sm:flex-row">
                        <a className="inline-flex min-h-12 w-full items-center justify-center rounded-xl bg-slate-900 px-6 py-3 text-sm font-bold text-white transition hover:bg-slate-800 sm:w-auto" href="mailto:management@remaltourismllc.com">التواصل مع الدعم</a>
                        <a href="/" className="inline-flex min-h-12 w-full items-center justify-center rounded-xl border border-slate-200 bg-white px-6 py-3 text-sm font-bold text-slate-700 transition hover:bg-slate-50 hover:text-[var(--remal-blue)] sm:w-auto">العودة للرئيسية</a>
                    </div>
                </div>
            </main>
        );
    ...
    if (!selectedBooking) {
        return (
            <main className="flex min-h-screen items-center justify-center bg-[#F8FAFC] px-5 py-12 lg:px-10">
                <div className="w-full max-w-md rounded-3xl border border-slate-100 bg-white p-10 text-center shadow-sm">
                    ...
                    <p className="mt-2 text-sm font-medium text-slate-500">الرجاء العودة وتحديد عرض إقامة للمتابعة.</p>
                    <button type="button" onClick={onBack} className="mt-8 inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-[var(--remal-blue)] px-5 py-3.5 text-sm font-black text-white shadow-lg shadow-cyan-700/20 transition hover:bg-[var(--remal-navy)]"><ArrowRight size={18} />العودة للبحث</button>
                </div>
            </main>
        );
    }
    ...
    return (
        <main className="min-h-screen bg-[#F8FAFC] px-5 py-8 pb-32 lg:px-10 lg:pb-16">
            <div className="mx-auto max-w-7xl">
                <button type="button" onClick={onBack} className="group mb-8 inline-flex min-h-12 items-center gap-2 text-sm font-bold text-slate-500 transition-colors hover:text-[var(--remal-blue)]"><ArrowRight size={18} className="transition-transform group-hover:-translate-x-1" />العودة لاختيار الغرفة</button>

                <header className="mb-10 max-w-3xl">
                    <div className="mb-3 flex items-center gap-2 text-[var(--remal-blue)]"><ShieldCheck size={20} /><span className="eyebrow">إتمام الحجز بأمان</span></div>
                    <h1 className="text-3xl font-black tracking-tight text-slate-900 sm:text-4xl lg:text-5xl">بيانات الضيف والدفع</h1>
                    <p className="mt-3 text-base font-medium leading-7 text-slate-500">خطوتان واضحتان لتأكيد إقامتك: أدخل بيانات الضيوف، ثم راجع السعر وانتقل إلى بوابة الدفع الآمنة.</p>
                </header>

                {!canPay && <div role="status" className="mb-8 flex items-start gap-3 rounded-2xl border border-amber-200 bg-amber-50 p-5 text-amber-800 shadow-sm"><Info size={24} className="mt-0.5 shrink-0 text-amber-500" /><div><p className="font-bold text-amber-900">الدفع الإلكتروني غير متاح لهذا العرض حالياً.</p><p className="mt-1 text-sm font-medium leading-6">لم يتم إنشاء عملية دفع. يرجى اختيار عرض آخر أو <a className="font-bold underline" href="mailto:management@remaltourismllc.com">التواصل مع فريق الحجوزات</a>.</p></div></div>}
