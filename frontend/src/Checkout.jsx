...
import { ArrowRight, CalendarDays, Check, CreditCard, LoaderCircle, Mail, Phone, ShieldCheck } from 'lucide-react';
    ...
    return <main className="min-h-screen bg-remal-bg px-5 py-8 lg:px-10">
        <div className="mx-auto max-w-6xl">
            ...
            <div className="grid gap-7 lg:grid-cols-[1fr_22rem]">
                <form onSubmit={submitPayment} className="order-2 rounded-2xl bg-white p-6 shadow-sm sm:p-8 lg:order-1">
                    <div className="mb-7 flex items-center gap-3 border-b border-slate-100 pb-5"><div className="grid h-10 w-10 place-items-center rounded-full bg-remal-blue/10 text-remal-blue"><CreditCard size={19} /></div><div><h2 className="font-black">بيانات الضيف الرئيسي</h2><p className="mt-1 text-xs font-bold text-slate-400">تُستخدم هذه البيانات لتأكيد الحجز</p></div></div>
                    <div className="grid gap-5 sm:grid-cols-2">
                        <label className="text-sm font-black">الاسم الأول<input required name="firstName" value={guest.firstName} onChange={updateGuest} className="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 font-normal outline-none transition focus:border-remal-blue" /></label>
                        <label className="text-sm font-black">اسم العائلة<input required name="lastName" value={guest.lastName} onChange={updateGuest} className="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 font-normal outline-none transition focus:border-remal-blue" /></label>
                        <label className="text-sm font-black"><span className="inline-flex items-center gap-2">البريد الإلكتروني <Mail size={14} className="text-remal-blue" /></span><input required type="email" name="email" value={guest.email} onChange={updateGuest} className="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 font-normal outline-none transition focus:border-remal-blue" /></label>
                        <label className="text-sm font-black"><span className="inline-flex items-center gap-2">رقم الهاتف <Phone size={14} className="text-remal-blue" /></span><input required type="tel" name="phone" value={guest.phone} onChange={updateGuest} className="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 font-normal outline-none transition focus:border-remal-blue" /></label>
                    </div>
                    <label className="mt-5 block text-sm font-black">طلبات خاصة<textarea name="specialRequests" value={guest.specialRequests} onChange={updateGuest} rows="4" className="mt-2 w-full resize-none rounded-xl border border-slate-200 px-4 py-3 font-normal outline-none transition focus:border-remal-blue" placeholder="مثال: سرير أطفال أو تسجيل وصول متأخر" /></label>
                    ...
                    <p className="mt-4 flex items-center justify-center gap-2 text-center text-xs font-bold text-slate-400"><ShieldCheck size={15} className="text-emerald-600" /> بياناتك محمية ولا يتم تخزين بيانات البطاقة لدينا</p>
                </form>
