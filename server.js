...
// مسار تصدير وتنزيل قسيمة الحجز PDF بتنسيق احترافي ومماثل لـ Agoda مع اللمسة المرحة
app.get('/api/bookings/pdf/:reference', async (req, res) => {
    try {
        ...
        doc.fillColor('#1f3a40').fontSize(13).font('Helvetica-Bold').text(`Booking ID / رقم المرجع: ${booking.bookingReference}`, 50, 115);
        doc.fontSize(11).fillColor('#0077b6').text('Status: CONFIRMED ✅', 390, 116);
        doc.moveDown(2.5);

        // Details Section
        doc.fontSize(12).fillColor('#1f3a40').font('Helvetica-Bold').text('Reservation & Property Details / تفاصيل الحجز ومكان الإقامة:');
        doc.moveDown(0.5);

        const details = [
            ['Guest Name / اسم العميل:', booking.customerName],
            ['Property / الفندق المحجوز:', booking.hotelName],
            ['Email Address / الإيميل:', booking.email],
            ['Phone Number / رقم الهاتف:', booking.phone || 'غير متوفر'],
            ['Companions / المرافقين:', booking.companions || 'لا يوجد مرافقين'],
            ['Payment Method / طريقة الدفع:', booking.paymentMethod === 'visa' ? 'Credit Card (Visa)' : 'Pay at Hotel (الدفع بالاستقبال)'],
            ['Total Amount / المبلغ الإجمالي:', `${booking.price} AED`],
            ['Cancellation Policy / سياسة الإلغاء:', booking.cancellationPolicy]
        ];

        let startY = doc.y;
        details.forEach(item => {
            doc.fontSize(10).font('Helvetica-Bold').fillColor('#1f3a40').text(item[0], 50, startY, { width: 180 });
            doc.font('Helvetica').fillColor('#333333').text(item[1], 240, startY, { width: 300 });
            startY += 20;
        });
        ...
        // Important Notes
        doc.fontSize(11).font('Helvetica-Bold').fillColor('#ff595e').text('⚠️ Important Notes & Hotel Rules / ملاحظات هامة:');
        doc.moveDown(0.5);
        ...
        doc.text('• يرجى إبراز هذه القسيمة مع إثبات الشخصية عند مكتب الاستقبال.');
        doc.text('• تذكير ودي: ممنوع إدخال بطاطس حارة للغرفة، ونتمنى لكم إقامة ممتعة وسحبة موفقة عن الدوام! 😂');

        doc.moveDown(2);
        doc.fontSize(10).font('Helvetica-Bold').fillColor('#1f3a40').text('Authorized Stamp & Signature / ختم وتوقيع الشركة', { align: 'left' });
