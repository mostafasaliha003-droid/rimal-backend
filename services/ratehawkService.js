...
async function searchAvailability(rawParams = {}) {
    ...
    return serpHotels.filter(h => !isDeletedHotel(dbMap[h.hid] || dbMap[h.id])).map(h => {
        ...
        return {
            ...
            image: d.image || '',
            images: Array.isArray(d.staticData?.images) ? d.staticData.images : [],
            images_ext: Array.isArray(d.staticData?.images_ext) ? d.staticData.images_ext : [],
...
async function getHotelPricing(hotelId, searchParams = {}) {
    ...
    const raw = {
        ...
        images_ext: Array.isArray(doc?.staticData?.images_ext) ? doc.staticData.images_ext : [],
